...
pub struct Line {
    ...
    /// leaves the glyphs small. `ink_top` is the first such row.
    pub ink_top: u32,
    pub ink_height: u32,
}

/// Font fallback order: Inter, then emoji. Fastpotify links system script
/// fallbacks on top; ytamp ships the two faces it needs and lets whatever
/// is still uncovered draw as the question mark, as Winamp did.
...
mod tests {
    ...
    #[test]
    fn emojis_are_drawn_from_the_bundled_emoji_face() {
        let mut text = PixelText::default();
